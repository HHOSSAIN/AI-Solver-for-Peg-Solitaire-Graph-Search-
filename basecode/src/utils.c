...
void execute_move_t(state_t* state, position_s* selected_peg, move_t jump) {
    ...
    //updating the look of the board
    switch (jump) {
    case up:          //Jump up
        state->field[x][y-2] = 'o'; 
        state->field[x][y-1] = '.'; 
...
//checks if cursor is on a peg or not,if not,then returns 0
...
/* called after play_solution() irrespective of whether play_solution() was invoked or not */
void print_solution(){
	...
	for(int i=0; i <= solution_size; i++){
		...
		if( i < solution_size){
			...
			switch ( solution_moves[i] ) {
				case up:          
					solution[i].cursor.y = solution[i+1].cursor.y+2; //parent er cursor val is 2 ghor down
...
/* Functions below provided by Alistair Moffat and modified by Hasne according
 * to need */
...
list_t
*insert_at_foot(list_t *list, node_t* n) {
    assert(list!=NULL && n!=NULL);
    if (list->foot==NULL) {
        list->head = list->foot = n;
    } else {
        list->foot->next = n;
        list->foot = n;
    }
